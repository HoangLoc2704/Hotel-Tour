        ...
        <form method="POST" action="{{ route('nhan-vien.store') }}">
            ...
            <div class="row">
                <div class="col-md-6 mb-3">
                    ...
                    <input type="text" class="form-control @error('TenNV') is-invalid @enderror" id="TenNV" name="TenNV" value="{{ old('TenNV') }}" required>
                    @error('TenNV')
            ...
            <div class="row">
                <div class="col-md-6 mb-3">
                    ...
                    <input type="text" class="form-control @error('SDT') is-invalid @enderror" id="SDT" name="SDT" value="{{ old('SDT') }}" maxlength="10" required>
                    @error('SDT')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <select class="form-select @error('MaCV') is-invalid @enderror" id="MaCV" name="MaCV" required>
                        ...
                        @foreach ($chucVu as $cv)
                            <option value="{{ $cv->MaCV }}" {{ old('MaCV') == $cv->MaCV ? 'selected' : '' }}>{{ $cv->TenCV }}</option>
                        @endforeach
                    </select>
                    @error('MaCV')
                        <div class="invalid-feedback">{{ $message }}</div>
            ...
            <div class="mb-3">
                ...
                <textarea class="form-control @error('DiaChi') is-invalid @enderror" id="DiaChi" name="DiaChi" rows="3" required>{{ old('DiaChi') }}</textarea>
                @error('DiaChi')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
