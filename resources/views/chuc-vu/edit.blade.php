...
<div class="row mb-4">
    <div class="col-md-8">
        <h2 class="dashboard-title">Sửa chức vụ</h2>
    </div>
...
<div class="card">
    <div class="card-body">
        <form id="chucVuEditForm" method="POST" action="{{ route('chuc-vu.update', $chucVu->MaCV) }}">
            @csrf
            @method('PUT')
            <div class="mb-3">
                <label for="TenCV" class="form-label">Tên chức vụ <span class="text-danger">*</span></label>
                <input type="text" class="form-control" id="TenCV" name="TenCV" value="{{ old('TenCV', $chucVu->TenCV) }}" required>
...
<script>
document.getElementById('chucVuEditForm').addEventListener('submit', async function (event) {
